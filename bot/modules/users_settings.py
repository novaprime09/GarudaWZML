...
handler_dict = {}
example_dict = {'prefix':'1. @your_channel_username or Anything', 'suffix':'1. ~ WZML\n2. ~ @channelname', 'caption':'1. <b>{filename}</b>\nJoin Now : @WeebZone_updates', 'userlog':'1. -100xxxxxx or Channel ID', 'remname':'', 'imdb_temp':'', 'ani_temp':''}
...
Check all available qualities options <a href="https://github.com/yt-dlp/yt-dlp#filtering-formats">HERE</a>.
        ...
        buttons.sbutton("Back", f"userset {user_id} back")
        buttons.sbutton("Close", f"userset {user_id} close")
        editMessage(f'<u>Send {data[3].capitalize()} text :</u>\n\nExamples:\n{escape(example_dict[data[3]])}', message, buttons.build_menu(2) if menu else buttons.build_menu(1))
